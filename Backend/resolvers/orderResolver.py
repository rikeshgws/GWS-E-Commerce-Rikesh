...
from utils.helper import get_current_user, is_buyer, is_seller
from inputtype.common import GenericResponse
from fastapi import HTTPException
import json

async def get_buyer_orders_resolver(context):
    db = SessionLocal()
    try:
        ...
        return GenericResponse(success=False, message=f"Failed to fetch orders: {str(e)}")
    finally:
        db.close()

async def get_seller_orders_resolver(context):
    db = SessionLocal()
    try:
        user_id = get_current_user(context)
        user = db.query(User).filter(User.id == user_id).first()
...
async def place_order_resolver(input, context):
    db = SessionLocal()
    try:
        user_id = get_current_user(context)
...
async def mark_order_delivered_resolver(order_id, context):
    db = SessionLocal()
    try:
        user_id = get_current_user(context)
        buyer = db.query(User).filter(User.id == user_id).first()
